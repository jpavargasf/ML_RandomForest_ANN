...
vlinear_disc, vlinear_se,vlinear_classes,vlinear_noelem = data_manipulation.dicretize_data(vlinear,disc_vlinear_range)
vangular_disc, vangular_se,vangular_classes,vangular_noelem = data_manipulation.dicretize_data(vangular,disc_vangular_range)

plt.stem(np.sort(vlinear))
plt.title("Velocidade linear ordenada")
plt.ylabel("Velocidade linear [m/s]")
plt.xlabel("Posição dos dados ordenados")
plt.show()

plt.stem(np.sort(vlinear_disc))
plt.ylabel("Velocidade linear [m/s]")
plt.title("Velocidade linear discretizada")
plt.xlabel("Posição dos dados ordenados")
plt.show()

plt.stem(np.sort(vlinear_se))
plt.title("Erro quadrático ordenado da velocidade linear discretizada")
plt.ylabel("Erro quadrático [m/s]^2")
plt.xlabel("Posição dos erros ordenados")
plt.show()

plt.stem(np.sort(vangular))
plt.ylabel("Velocidade angular [rad/s]")
plt.title("Velocidade angular")
plt.xlabel("Posição dos dados ordenados")
plt.show()

plt.stem(np.sort(vangular_disc))
plt.ylabel("Velocidade angular [rad/s]")
plt.title("Velocidade linear discretizada")
plt.xlabel("Posição dos dados ordenados")
plt.show()

plt.stem(np.sort(vangular_se))
plt.title("Erro quadrático ordenado da velocidade angular discretizada")
plt.ylabel("Erro quadrático [rad/s]^2")
plt.xlabel("Posição dos erros ordenados")
plt.show()
